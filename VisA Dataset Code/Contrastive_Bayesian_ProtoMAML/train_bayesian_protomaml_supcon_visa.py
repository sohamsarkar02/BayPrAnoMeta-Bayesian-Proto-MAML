#!/usr/bin/env python3
"""
Contrastive BayPrAnoMeta (ViSA): Bayesian ProtoMAML + Supervised Contrastive
CPU friendly
"""
...
OUT_DIR = "runs/bpmaml_supcon_visa"                     
...
def run_episode(model, data, train=True):
    ...
    xq = torch.stack(load_images(
        qn + qa,
        transform_train if train else transform_eval
    )).to(DEVICE)
...
def main():
    ...
    datasets = {}
    for c in CLIENTS:
        d = load_client_dataset_visa(                
            Path(BASE_ROOT)/c, seed=SEED
        )
        if d is not None:
            datasets[c] = d
        else:
