#!/usr/bin/env python3
"""
Evaluation for Contrastive BayPrAnoMeta: Bayesian ProtoMAML + Supervised Contrastive Learning
CPU friendly
"""
...
MODEL_PATH = "runs/bpmaml_supcon_leave1out/checkpoints/final_model.pth"
OUT_DIR = "runs/bpmaml_supcon_leave1out/eval"
...
def evaluate_client(model, client):
    ...
    data = load_client_dataset_leave1out(Path(BASE_ROOT) / client, seed=42)
    if data is None:
    ...
    plt.title(f"{client} | Held-out: {data['held_out_subtype']}")
    plt.tight_layout()
    ...
    return {
        ...
        "f1_mean": f1_m,
        "f1_se": f1_se,
        "held_out_subtype": data["held_out_subtype"]
    }
