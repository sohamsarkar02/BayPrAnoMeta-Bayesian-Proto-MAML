#!/usr/bin/env python3
"""
Federated BayPrAnoMeta: Federated Bayesian Proto-MAML
CPU friendly
"""
...
def make_episode(good_imgs, anom_imgs):
    support = random.sample(good_imgs, K_SHOT)
    q_norm = random.sample(good_imgs, Q_N)
    q_anom = random.sample(anom_imgs, Q_A)
    return support, q_norm, q_anom
...
def client_compute_gradient(global_model, data):
    ...
    for _ in range(EPISODES_PER_CLIENT):
        ...
        with higher.innerloop_ctx(local_model, inner_opt, copy_initial_weights=True) as (fmodel, diffopt):
            ...
            outer_loss = -((1-labels)*logp_n + labels*logp_a).mean()

            grads = torch.autograd.grad(outer_loss, fmodel.parameters())
...
def main():
    ...
    for c in CLIENTS:
        ...
        anom_imgs = []
        for d in anoms[:-1]:
            anom_imgs += list_images(d)

        client_data[c] = {
            "good_train": good,
            "anom_train": anom_imgs,
            "good_test": good,
